...
const SourcesScanModal: React.FC<SourcesScanModalProps> = ({
    sources,
    onClose,
    onSubmit
}) => {
              ...
              <Form isHorizontal>
                <FormGroup label="Name" isRequired fieldId="scan-name">
                  <TextInput
                    ...
                    id="scan-name"
                    name="scan-name"
                    onChange={(ev) => { setValue('scan-name', (ev.target as HTMLInputElement).value)}}
                  />
                </FormGroup>
                ...
                <FormGroup label="Maximum concurrent scans" fieldId="scan-max-concurrent">
                    <NumberInput
                        ...
                        onMinus={() => setValue('scan-max-concurrent', "" + Math.max(1, Number(getValue('scan-max-concurrent')) - 1))}
                        onChange={(ev) => setValue('scan-max-concurrent', "" + Math.max(1, Math.min(200, Number((ev.target as HTMLInputElement).value))))}
                        onPlus={() => setValue('scan-max-concurrent', "" + Math.min(200, Number(getValue('scan-max-concurrent')) + 1))}
                        inputName="input"
                ...
                {!!deepScans.length && <FormGroup label="Scan alternate directories" fieldId="scan-alt-scan">
                  <TextArea
                ...
                <ActionGroup>
                  <Button variant="primary" onClick={() => onScan({...values, deepScans})}>Save</Button>
